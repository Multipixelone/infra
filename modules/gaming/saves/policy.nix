# The Core policy and the three Client profiles, as data.
#
# Kept in this repository rather than a dedicated private Forgejo repository.
# The extraction boundary is written down in
# docs/retroarch-save-authority-runbook.md and is deliberately not crossed yet:
# nothing here is a credential, a Library inventory, or a content hash, so the
# material is no more sensitive than the rest of this tree. The moment the
# policy starts naming games a third party should not learn about, or grows a
# release cadence of its own, it earns its own repository.
#
# The system keys ARE the Library's platform directory names, read off
# /media/Data/romm/library/roms on link. That is not a coincidence to be tidied
# up later: sort_savefiles_by_content_enable derives the save directory from the
# ROM's parent directory name, so this string is simultaneously the Library
# directory, every client's ROM subdirectory, and the remote save prefix. Change
# one and you have silently forked a game's save history.
{
  saveSync = {
    corePolicy = {
      systems = {
        gb = {
          ...
          romExtensions = [ "gb" ];
          # .rtc is Gambatte's real-time-clock sidecar for the Pokémon Gen 2
          # carts; it is as recovery-critical as the .srm it sits beside and is
          # exactly the kind of extension a hand-written reject list forgets.
          saveExtensions = [
            "srm"
            "rtc"
            "sav"
        ...
        gba = {
          # The pilot core. Moves GBA off gpSP, which wrote most of the existing
          # saves: mGBA is the accurate core, and the two disagree about GBA
          # flash-save sizing often enough that a save must be proven under mGBA
        ...
        nds = {
          ...
          romExtensions = [ "nds" ];
          # .dsv is DeSmuME's format and .srm is melonDS's for the same
          # cartridge. Both are present in the live save tree for Pokémon White,
          # which is the concrete reason "never assume battery saves are
          # cross-core compatible" is a rule and not a caveat.
          saveExtensions = [
            "dsv"
            "sav"
            "srm"
      ...
      # No per-game exceptions today. The mechanism exists and is tested against
      # synthetic fixtures in checks.retroarch-saves-core-policy; adding a real
      # one here without a real reason would be inventing policy.
      exceptions = { };

      # gpSP wrote most of the GBA saves now on disk and bsnes wrote two of the
      # SNES ones. Neither is prescribed for any system any more, but both stay
      # installed so those saves can still be opened, verified and exported
    ...
    # Only games the policy names. Not a Library inventory: 42 GBA ROMs exist
    # and exactly one of them appears here, because exactly one is referenced by
    # a profile.
    contentIdentities."gba/Pokemon - Emerald-R 260525" = {
      title = "Pokémon Revelation (v260525)";
      why = ''
        The pilot title is RomM ROM ID 110, whose exact filename is
        "Pokemon - Emerald-R 260525.gba". Upstream calls this Pokémon Revelation
        v260525, but the content identity deliberately retains the exact RomM
        filename stem because RetroArch matches saves by that stem.

        Legacy v231014 saves are not assumed compatible. Retain the old ROM and
        saves, and copy one only for an explicit copy-and-verify compatibility
        test under mGBA before it is considered for this identity.
      '';
    };

    clients = {
      # link reads the Library directly and receives no ROM dataset of its own;
      # its ROM path IS the Library. It enables every emulated system so the
      # cores it already has stay installed.
      link = {
        platform = "linux";
        managed = true;
        systems = [
        ...
        paths = {
          roms = "/media/Data/romm/library/roms";
          bios = "/media/Data/romm/library/bios";
          ...
          system = "~/.config/retroarch/system";
          coreAssets = "~/.config/retroarch/downloads";
        };
      };
      ...
      ios = {
        ...
        # App Store RetroArch is sandboxed; these are the on-device paths
        # relative to the app's Documents container, which is what Files and
        # Möbius see.
        paths = {
          roms = "RetroArch/roms";
          bios = "RetroArch/system";
          saves = "RetroArch/saves";
          states = "RetroArch/states";
          playlists = "RetroArch/playlists";
          config = "RetroArch/config";
          system = "RetroArch/system";
          coreAssets = "RetroArch/downloads";
        };
      };
    ...
    webdav = {
      # Trailing slash on purpose. RetroArch normalises it away itself
      # (fill_pathname_slash in webdav_sync_begin), but a URL without an
      # explicit scheme defaults to http://, so the scheme is what actually
      # matters here and is written out in full on every client.
      publicUrl = "https://saves.finnrut.is/";
      port = 8385;
      dataDir = "/media/Data/retroarch-saves";
      snapshotDir = "/media/Data/.retroarch-save-snapshots";
    };

    pilot = {
      enable = true;
      emeraldContentIdentity = "gba/Pokemon - Emerald-R 260525";
    };

    # All three flipped together on 2026-09-07, after the diff of the live file
    # against the real generated config came back
    #   added 1   changed 15   dropped 11
    # with all 11 drops being credential keys -- ten of them already empty in
    # the live file, and only cheevos_token holding a value. Nothing of the 3303
    # imported settings is lost.
    #
    # They flip together because they are one decision, not three:
    # manageConfig without injectCredentials is refused by an assertion (nix
    # would own a config with cloud_sync_enable = "true" and no credentials, and
    # RetroArch has no retry, so every sync would fail silently forever), and
    # injectCheevos without a cheevos_username in baseSettings is refused by
    # another (a password with no account name never authenticates and nothing
    # says so). baseSettings supplies cheevos_username = "tunnelmaker".
    #
    # The price, stated plainly: config_save_on_exit is now "false", so every
    # persistent RetroArch UI change from here is an edit to this file. That is
    # the whole point -- it is what stopped libretro_directory pointing at a
    # 1.20.0 store path while the binary was 1.22.2 -- but it is a real change
    # in how this machine is used.
    retroarch.manageConfig = true;
    retroarch.injectCredentials = true;
    retroarch.injectCheevos = true;

    retroarch.baseSettings = {
      # Generated by retroarch-config-import from /home/tunnel/.config/retroarch/retroarch.cfg
      # 3303 of 3333 settings; the rest are credentials or managed keys.
      ...
      "assets_directory" =
        "/nix/store/ww5327c6m9zjzwbii1ii2981shw87wz2-retroarch-assets-1.20.0-unstable-2025-01-30/share/retroarch/assets";
      ...
      "input_overlay_analog_recenter_zone" = "0";
      ...
      "input_player10_device_reservation_type" = "0";
      ...
      "input_player10_gun_offscreen_shot_mbtn" = "2";
      ...
      "input_player11_device_reservation_type" = "0";
      ...
      "input_player11_gun_offscreen_shot_mbtn" = "2";
      ...
      "input_player11_reserved_device" = "";
      ...
      "input_player12_device_reservation_type" = "0";
      ...
      "input_player12_gun_offscreen_shot_mbtn" = "2";
      ...
      "input_player12_reserved_device" = "";
      ...
      "input_player13_device_reservation_type" = "0";
      ...
      "input_player13_gun_offscreen_shot_mbtn" = "2";
      ...
      "input_player13_reserved_device" = "";
      ...
      "input_player14_device_reservation_type" = "0";
      ...
      "input_player14_gun_offscreen_shot_mbtn" = "2";
      ...
      "input_player15_device_reservation_type" = "0";
      ...
      "input_player15_gun_offscreen_shot_mbtn" = "2";
      ...
      "input_player15_reserved_device" = "";
      ...
      "input_player16_device_reservation_type" = "0";
      ...
      "input_player16_gun_offscreen_shot_mbtn" = "2";
      ...
      "input_player16_reserved_device" = "";
      ...
      "input_player5_device_reservation_type" = "0";
      ...
      "input_player5_gun_offscreen_shot_mbtn" = "2";
      ...
      "input_player6_device_reservation_type" = "0";
      ...
      "input_player6_gun_offscreen_shot_mbtn" = "2";
      ...
      "input_player7_device_reservation_type" = "0";
      ...
      "input_player7_gun_offscreen_shot_mbtn" = "2";
      ...
      "input_player8_device_reservation_type" = "0";
      ...
      "input_player8_gun_offscreen_shot_mbtn" = "2";
      ...
      "input_player9_device_reservation_type" = "0";
      ...
      "input_player9_gun_offscreen_shot_mbtn" = "2";
      ...
      "joypad_autoconfig_dir" =
        "/nix/store/c9l77bv98m3qj29jp7b7cbb2nnic2hmg-retroarch-joypad-autoconfig-1.20.0/share/libretro/autoconfig";
      "keyboard_gamepad_enable" = "true";
      "keyboard_gamepad_mapping_type" = "1";
      "kiosk_mode_enable" = "false";
      "led_driver" = "null";
      # NOT the imported value. The live file pointed this at a
      # retroarch-with-cores-1.20.0 store path that no longer exists while the
      # running binary was 1.22.2 -- the exact drift the runbook's import step
      # says to remove rather than preserve. A store path is the wrong shape
      # here regardless of version: it is garbage-collectable, and the menu
      # scans this directory even though the launcher passes -L. The per-user
      # profile symlink is GC-rooted and follows generations.
      "libretro_directory" = "/home/tunnel/.nix-profile/lib/retroarch/cores";
      "libretro_info_path" =
        "/nix/store/xd0iz8spiqh8ny4wv1nwyihnyxqz7aiw-libretro-core-info-1.20.0/share/retroarch/cores";
      ...
      # NOT the imported "default". link's paths.roms IS the RomM Library, and
      # the file browser is where a human picks the ROM whose parent directory
      # then names the save prefix under sort_savefiles_by_content_enable.
      # Leaving this at "default" is what let the pilot get loaded out of
      # ~/.config/retroarch/roms instead, and a browser rooted anywhere else is
      # one wrong directory name away from forking a game's save history.
      "rgui_browser_directory" = "/media/Data/romm/library/roms";

...
}:
{
  ...
  configurations.nixos.link.module =
    ...
    let
      ...
      inherit (openclawMusicStreamrip) launcher masterConfig runtimeRoot;
      ...
      openclaw-music = pkgs.writeShellApplication {
        ...
        text = ''
          ...
          exec ${pkgs.coreutils}/bin/env -i \
            ...
            OPENCLAW_MUSIC_STREAMRIP_LAUNCHER=${lib.getExe launcher} \
            OPENCLAW_MUSIC_STREAMRIP_MASTER_CONFIG=${masterConfig} \
            OPENCLAW_MUSIC_STREAMRIP_RUNTIME_ROOT=${runtimeRoot} \
            ${lib.getExe package} "$@"
        '';
      };
    ...
    in
    {
      assertions = [
        ...
        {
          assertion = lib.all (path: nonOverlapping runtimeRoot path) [
            ledger-root
            staging-root
            library-root
            download-root
          ...
          message = "openclaw-music Streamrip runtime must remain isolated from worker paths";
        }
      ];
